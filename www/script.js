...
function render() {
    ...
    sorted.forEach((act, index) => {
        ...
        if (pct > 100) {
            ...
            else if (pct >= 75) level = 75;
            else if (pct >= 50) level = 50;
            else if (pct >= 25) level = 25;
            colorClass = isMalus ? {0:'p-100', 25:'p-75', 50:'p-50', 75:'p-25', 100:'p-0'}[level] : `p-${level}`;
        }

        // Gouttes UNIQUEMENT pour malus
        let dripHTML = '';
        if (isMalus && pct > 100) {
            dripHTML = '<div class="drip left"></div><div class="drip right"></div>';
        }
            ...
            for (let i = 0; i < 5; i++) {
                ...
                bubblesHTML += `
                    <div class="bubble" style="
                        ...
                        height: ${size}px;
                        left: ${left}%;
                        bottom: ${Math.random() * Math.min(pct, 100)}%;
                        animation-delay: ${delay}s;
                    "></div>`;
            }
        }

        // Effets permanents pour débordement
        let effectsHTML = '';
        if (pct > 100) {
            if (isMalus) {
                // Effets malus : têtes de mort et particules de lave
                for (let i = 0; i < 3; i++) {
                    const left = Math.random() * 70 + 15;
                    const delay = Math.random() * 2;
                    effectsHTML += `
                        <div class="skull" style="
                            left: ${left}%;
                            animation-delay: ${delay}s;
                        ">💀</div>`;
...
// NOUVELLE FONCTION : Synchronisation manuelle
function syncDay() {
    const now = new Date();
    const today = now.toISOString().split('T')[0];
    
    // Vérifier si on a déjà synchronisé aujourd'hui
    if (state.lastDailyReset === today) {
        alert("✅ Vous êtes déjà synchronisé avec aujourd'hui !\n\nLes vases ont déjà été réinitialisés pour cette journée.");
        return;
    }
    
    // Sauvegarder l'état d'avant la synchronisation pour le message
    let message = "🔄 Synchronisation réussie !\n\n";
    let overflowMessages = [];
    
    // Calculer les streaks avant de réinitialiser
    const bonusVases = state.activities.filter(a => a.type === 'bonus');
    const malusVases = state.activities.filter(a => a.type === 'malus');
    
    // Calculer les streaks globaux
    if (bonusVases.length > 0 && bonusVases.every(a => (a.dCurr / a.dTarget) >= 1.0)) {
        state.globalStreak++;
        message += `🔥 Nouvelle série : ${state.globalStreak} jours consécutifs !\n`;
    } else {
        state.globalStreak = 0;
    }
    
    if (malusVases.length > 0 && malusVases.every(a => (a.dCurr / a.dTarget) < 1.0)) {
        state.malusStreak++;
        message += `💪 Résistance malus : ${state.malusStreak} jours !\n`;
    } else {
        state.malusStreak = 0;
    }
    ...
    state.activities.forEach(act => {
        ...
        const ratio = act.dTarget > 0 ? act.dCurr / act.dTarget : 0;
        
        // Mettre à jour le streak individuel
        if (act.type === 'bonus') {
            act.streak = (ratio >= 1.0) ? (act.streak || 0) + 1 : 0;
        } else {
            act.streak = (ratio < 1.0) ? (act.streak || 0) + 1 : 0;
        }
        
        // Gérer le surplus pour les bonus seulement
        if (act.type === 'bonus' && act.dCurr > act.dTarget) {
            act.carry = act.dCurr - act.dTarget;
            overflowMessages.push(`🎯 ${act.name} : surplus de ${formatTime(act.carry)} reporté !`);
        } else {
            act.carry = 0; // Pas de surplus pour les malus
        }
        
        // Réinitialiser le vase
        act.dCurr = act.carry || 0; // Les bonus gardent leur surplus, les malus partent de 0
        
        // Mettre à jour la cible du jour avec le planning si disponible
        const weekID = getWeekID(now);
        const todayIdx = (now.getDay() + 6) % 7;
        const planned = (state.planning[weekID] && state.planning[weekID][todayIdx] && state.planning[weekID][todayIdx][act.id]);
        if (planned) {
            const oldTarget = act.dTarget;
            act.dTarget = planned;
            if (oldTarget !== planned) {
                overflowMessages.push(`📅 ${act.name} : objectif ajusté à ${formatTime(planned)}`);
            }
        }
    });
    
    // Ajouter les messages de débordement
    if (overflowMessages.length > 0) {
        message += "\n" + overflowMessages.join('\n');
    } else {
        message += "\n✨ Tous les vases ont été vidés pour la nouvelle journée !";
    }
    
    // Mettre à jour la date
    state.lastDailyReset = today;
    localStorage.setItem('lastDailyReset', today);
    localStorage.setItem('globalStreak', state.globalStreak);
    localStorage.setItem('malusStreak', state.malusStreak);
...
// FONCTION checkAndReset MODIFIÉE (version originale améliorée)
function checkAndReset() {
    const now = new Date();
    const today = now.toISOString().split('T')[0];
    const weekID = getWeekID(now);
    
    // Si c'est la première fois ou si un jour est passé
    if (state.lastDailyReset !== "" && state.lastDailyReset !== today) {
        ...
        // Pour chaque activité
        state.activities.forEach(act => {
            const r = act.dTarget > 0 ? act.dCurr / a.dTarget : 0;
            
            // Mettre à jour les streaks individuels
            if (act.type === 'bonus') {
                act.streak = (r >= 1.0) ? (act.streak || 0) + 1 : 0;
            } else {
                act.streak = (r < 1.0) ? (act.streak || 0) + 1 : 0;
            }
            
            // Gérer le surplus (uniquement pour les bonus)
            if (act.type === 'bonus' && act.dCurr > act.dTarget) {
                act.carry = act.dCurr - act.dTarget;
            } else {
                act.carry = 0; // Pas de surplus pour les malus
            ...
            // NE PAS réinitialiser automatiquement - laisser l'utilisateur le faire avec le bouton synchroniser
            // act.dCurr = act.carry || 0;
            
            // Mettre à jour l'objectif du jour avec le planning si disponible
            const tIdx = (now.getDay() + 6) % 7;
            const planned = (state.planning[weekID] && state.planning[weekID][tIdx] && state.planning[weekID][tIdx][act.id]);
            if (planned) {
                act.dTarget = planned;
            }
        });
        
        // Sauvegarder les streaks
        localStorage.setItem('globalStreak', state.globalStreak);
        localStorage.setItem('malusStreak', state.malusStreak);
    }
    
    // Initialiser si premier jour
    if (state.lastDailyReset === "") {
        state.lastDailyReset = today;
        localStorage.setItem('lastDailyReset', today);
    }
    
    // Toujours mettre à jour les objectifs du jour avec le planning
    const todayIdx = (now.getDay() + 6) % 7;
    state.activities.forEach(act => {
        const planned = (state.planning[getWeekID(now)] && state.planning[getWeekID(now)][todayIdx] && state.planning[getWeekID(now)][todayIdx][act.id]);
        if (planned) {
            act.dTarget = planned;
        }
    });
...
function saveTime() {
    ...
    if (state.scale === 'daily') {
        act.dCurr = Math.max(0, act.dCurr + mins);
        act.wCurr = Math.max(0, act.wCurr + mins);
        act.dTarget = Math.max(1, act.dTarget + tMins);
        act.wTarget = Math.max(1, act.wTarget + tMins);
        
        if (act.dCurr > act.dTarget && act.type === 'bonus') {
            act.carry = act.dCurr - act.dTarget;
            document.getElementById('overflowMsg').innerHTML = `L'objectif <b>${act.name}</b> a explosé !<br>Le surplus de <b>${formatTime(act.carry)}</b> sera reporté à demain lors de la synchronisation. 🌈`;
